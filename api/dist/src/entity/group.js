"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Group = void 0;
const typeorm_1 = require("typeorm");
const lnkGroupPermission_1 = require("./lnkGroupPermission");
const lnkGroupUser_1 = require("./lnkGroupUser");
let Group = class Group {
};
__decorate([
    (0, typeorm_1.PrimaryGeneratedColumn)('increment', { name: 'id_group' }),
    __metadata("design:type", Number)
], Group.prototype, "id", void 0);
__decorate([
    (0, typeorm_1.Column)('varchar', { length: 100, nullable: false }),
    __metadata("design:type", String)
], Group.prototype, "name", void 0);
__decorate([
    (0, typeorm_1.Column)("tinyint", { nullable: false, default: true }),
    __metadata("design:type", Boolean)
], Group.prototype, "status", void 0);
__decorate([
    (0, typeorm_1.OneToMany)(() => lnkGroupPermission_1.LnkGroupPermission, lnk => lnk.group, { cascade: ['insert', 'update'], nullable: false }),
    __metadata("design:type", Array)
], Group.prototype, "permission", void 0);
__decorate([
    (0, typeorm_1.OneToMany)(() => lnkGroupUser_1.LnkGroupUser, lnk => lnk.group, { cascade: ['insert', 'update'], nullable: false }),
    __metadata("design:type", Array)
], Group.prototype, "user", void 0);
__decorate([
    (0, typeorm_1.Column)('tinyint', { name: 'deleted', nullable: false, default: false, select: false }),
    __metadata("design:type", Boolean)
], Group.prototype, "deleted", void 0);
__decorate([
    (0, typeorm_1.CreateDateColumn)({ default: () => 'CURRENT_TIMESTAMP', name: 'create_date', precision: 0, nullable: false, select: false }),
    __metadata("design:type", Date)
], Group.prototype, "createDate", void 0);
__decorate([
    (0, typeorm_1.UpdateDateColumn)({ default: () => 'CURRENT_TIMESTAMP', name: 'update_date', precision: 0, onUpdate: 'CURRENT_TIMESTAMP', nullable: false, select: false }),
    __metadata("design:type", Date)
], Group.prototype, "updateDate", void 0);
Group = __decorate([
    (0, typeorm_1.Entity)('cad_group')
], Group);
exports.Group = Group;
//# sourceMappingURL=group.js.map